...
class ResultadoFalha(Base):
    ...
    id_resultado = relationship(
        Resultados, backref=backref("resultado_falhas"))
...
class PesquisaResultados(Base):
    ...
    id_pesquisa = relationship(
        Pesquisa, backref=backref("pesquisa_resultados"))
    id_resultado = relationship(
        Resultados, backref=backref("pesquisa_resultados"))
...
class ResultadoTelefone(Base):
    ...
    id_resultado = relationship(
        Resultados, backref=backref("resultados_telefone"))
    # Status:
    # 0 - Não avaliado
    # 1 - Valido
    # 2 - Invalido
...
class ResultadoEmail(Base):
    ...
    id_resultado = relationship(
        Resultados, backref=backref("resultados_email"))
    # Status:
    # 0 - Não avaliado
    # 1 - Valido
    # 2 - Invalido
